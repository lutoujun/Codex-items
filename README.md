...
本目录用于承载多个独立的数据开发与分析项目。业务文件按项目归集；第三方工具、缓存和可再生成产物不与业务源码混放。
...
- `_tools`：第三方工具源码，仅用于本地辅助。
- `_generated`：缓存和可再生成文件；不作为业务交付物。
...
4. 可再生成内容放入 `_generated` 或工具约定目录，并保持在 `.gitignore` 中。
